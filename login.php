<?php 
	...
	if($_POST) {
		if($_POST['email'] && $_POST['password']) {
			...
			if(isset($user->user_id) && $user->user_password == $password && ($email == $user->user_email || $email == $user->user_first)) {
				//set the session user of the site
				//load start session for storage
				$_SESSION['user_email'] = $user->user_email;
				//if successfull login take them to the main site
				header('Location: index');
				
			} else {
				if(!isset($user->user_id)) {
					$splash = $_['login_no_user'];
				} else {
					if(md5(($user->user_password) != $password)) {
						$splash = $_['login_wrong_password'];
					}
					if($email != $user->user_email && $email != $user->user_first) {
						$splash = $_['login_no_user'];
					}
				}
			}
...
<head>
	<meta charset="utf-8" />
	<title><?php echo $_['global_company_name']; ?> | Login Page</title>
...
<body class="login">
	...
	<div class="content">
		...
		<form class="form-vertical login-form" action="login" method="post">
			<h3 class="form-title"><?php echo $_['login_login_to_account'] ?></h3>
			<?php if($splash != '')  { ?>
			<div class="alert alert-error">
				<button class="close" data-dismiss="alert"></button>
				<span><?php echo $splash; ?></span>
			</div>
			<?php } ?>
			<form action="index.php" method="post">
			<div class="control-group">
				<!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
				<label class="control-label visible-ie8 visible-ie9"><?php echo $_['epost']; ?></label>
				<div class="controls">
					<div class="input-icon left">
						<i class="icon-user"></i>
						<input class="m-wrap placeholder-no-fix" type="text" autocomplete="off" placeholder="<?php echo $_['email']; ?>" name="email"/>
					</div>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label visible-ie8 visible-ie9"><?php echo $_['password']; ?></label>
				<div class="controls">
					<div class="input-icon left">
						<i class="icon-lock"></i>
						<input class="m-wrap placeholder-no-fix" type="password" autocomplete="off" placeholder="<?php echo $_['password']; ?>" name="password"/>
					</div>
				</div>
			</div>
			...
			<div class="form-actions">
				...
				<input type="checkbox" name="remember" value="1"/> <?php echo $_['login_remember_me']; ?> 
				</label>-->
				<button type="submit" class="btn green pull-right">
				<? echo $_['login_login']; ?> <i class="m-icon-swapright m-icon-white"></i>
				</button>            
			</div>
			</form>
			<!-- 
